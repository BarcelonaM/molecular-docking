...
from InterFocusGT.model.model2 import get_batch_num_nodes
...
def mdn_loss_fn(pi, sigma, mu, y, eps=1e-10): # y : batch , n_l , n_p
    normal = Normal(mu, sigma) # batch , n_l , n_p , n高斯
...
def run_a_train_epoch(epoch, model, data_loader, optimizer, aux_weight=0.001, device='cpu'):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        if np.isinf(mdn_loss) or np.isnan(mdn_loss):
            break
        del b_hetero_g, atom_labels, bond_labels, pi, sigma, mu, dist, atom_types, bond_types, batch, mdn, atom, bond, loss
        th.cuda.empty_cache()
...
def run_an_eval_epoch(model, data_loader, pred=False, atom_contribution=False, res_contribution=False, dist_threhold=None, aux_weight=0.001, device='cpu'):
    ...
    with th.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            total_dist += dist.shape[0]
            pl_in_dist += th.where(dist <= dist_threhold)[0].shape[0]


            if pred or atom_contribution or res_contribution:
                prob = calculate_probablity(pi, sigma, mu, dist) # batch , leg l , pro l
                ...
                if atom_contribution or res_contribution:
                    contribs = [prob[batch==i].reshape((  get_batch_num_nodes(b_hetero_g , 'ligand')[i],  get_batch_num_nodes(b_hetero_g , 'protein')[i])) for i in range(b_hetero_g.batch_size)]
                    if atom_contribution:
                        at_contrs.extend([contribs[i].sum(1).cpu().detach().numpy() for i in range(b_hetero_g.batch_size)])
                    if res_contribution:
                        res_contrs.extend([contribs[i].sum(0).cpu().detach().numpy() for i in range(b_hetero_g.batch_size)])

            else:
                mdn = mdn_loss_fn(pi, sigma, mu, dist)
                # mdn_loss_5 = mdn.detach().clone()[th.where(dist <=   dist_threhold)[0]].mean().item() * b_hetero_g.batch_size
                mdn = mdn[th.where(dist <= model.dist_threhold)[0]]
                mdn = mdn.mean()
                atom = F.cross_entropy(atom_types, atom_labels)
                bond = F.cross_entropy(bond_types, bond_labels)
        ...
        print("pl dist in ratio{}/{}={}".format(pl_in_dist,total_dist,pl_in_dist/(total_dist+0.001)))
...
def print_hetero(hg):
    try:
        print("hg.nodes['protein'].data['feat']" , hg.nodes['protein'].data['feat'].shape)
        print("hg.nodes['protein'].data['pos']" , hg.nodes['protein'].data['pos'].shape)
        print("hg.edges['p2p'].data['feat']" , hg.edges['p2p'].data['feat'].shape)
        print("hg.nodes['ligand'].data['feat']"  , hg.nodes['ligand'].data['feat'].shape)
        print("hg.nodes['ligand'].data['pos']" , hg.nodes['ligand'].data['pos'].shape)
        print("hg.edges['l2l'].data['feat']" , hg.edges['l2l'].data['feat'].shape)


        print("hg.edges['l2p'].data['feat']", hg.edges['l2p'])
        print("hg.edges['p2l'].data['feat']", hg.edges['p2l'])

...
def UserInput():
    ...
    p.add_argument('-process_num', '--process_num', default=99999999, type=int,
                   help='')
    ...
    p.add_argument('-test', '--test', default=False, action="store_true")
    args = p.parse_args()
    return args
...
def main():
    ...
    print('过滤前' , len(results))
    results = list(filter(lambda x: x[1] != None, results))
    print('过滤后' , len(results))
    ids, hetero_g  = list(zip(*results))
    if not args.test:
        np.save("{}\\{}_idsresz.npy".format(args.out_dir ,args.out_prefix ) , ids)

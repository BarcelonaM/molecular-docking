...
import os
from ..feats.mol2graph_rdmda_res import mol_to_graph, load_mol, prot_to_graph  , create_hetero_graph
...
class  HeteroPDBbindDataset(Dataset):
    def __init__(self,
        ...
        assert len(self.pdbids) == len(self.hetero_g) == self.labels.shape[0]

    def __getitem__(self, idx):
        return self.pdbids[idx], self.hetero_g[idx] , self.labels[idx]
    ...
    def add(self , data):
        if  self.pdbids is None :
            ...
            self.labels = th.concatenate((self.labels, data.labels))

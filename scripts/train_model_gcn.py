...
from InterFocusGCN.data.data import HeteroPDBbindDataset
from InterFocusGCN.model.model2 import InterFocusGCN, HeteroGatedGCN
from InterFocusGCN.model.utils import EarlyStopping, set_random_seed, run_a_train_epoch, run_an_eval_epoch
import argparse
import logging
...
p.add_argument('-affi_weight', '--affi_weight', type=float, default=0)
...
train_data = HeteroPDBbindDataset(ids=data.pdbids[train_inds],hetero_g= [data.hetero_g[idx] for idx in train_inds]  , labels=data.labels[train_inds],)
val_data = HeteroPDBbindDataset(ids=data.pdbids[val_inds],hetero_g=[data.hetero_g[idx]  for idx in  val_inds]  , labels=data.labels[val_inds], )
logger.info('数据划分后，训练样本数量/开发样本数量：{}/{}'.format(len(train_data),len(val_data)))

hetero_model = HeteroGatedGCN(
                                p_in_channels = 41,
                                p_edge_features=5,
...
model = InterFocusGCN(hetero_model,
...
for epoch in range(args["num_epochs"]):
    total_loss_train, mdn_loss_train, affi_loss_train, atom_loss_train, bond_loss_train = run_a_train_epoch(epoch, model, train_loader, optimizer, aux_weight=args["aux_weight"] , affi_weight = input_args.affi_weight , val_dist_threshold = args["val_dist_threhold"] , device=args["device"] )
    if np.isinf(mdn_loss_train) or np.isnan(mdn_loss_train):
        logger.info('Inf ERROR')
        break
    total_loss_val, mdn_loss_val,affi_loss_val, atom_loss_val, bond_loss_val = run_an_eval_epoch(model, val_loader, dist_threhold=args["val_dist_threhold"]  , affi_weight=input_args.affi_weight , aux_weight=args["aux_weight"], device=args["device"])
    early_stop = stopper.step(total_loss_val, model)
    logger.info('epoch {:d}/{:d} total_loss_val {:.4f}, mdn_loss_val {:.4f}, affi_loss_val {:.4f}, atom_loss_val {:.4f}, bond_loss_val {:.4f}, best validation {:.4f}'.format(epoch + 1, args['num_epochs'], total_loss_val, mdn_loss_val,affi_loss_val, atom_loss_val, bond_loss_val, stopper.best_score)) #+' validation result:', validation_result)
    if early_stop:
        break
...
total_loss_val, mdn_loss_val, affi_loss_val, atom_loss_val, bond_loss_val = run_an_eval_epoch(model, val_loader, dist_threhold=args["val_dist_threhold"], affi_weight=input_args.affi_weight , aux_weight=args["aux_weight"], device=args["device"])
logger.info(
    "total_loss_val:%s, mdn_loss_val:%s , affi_loss_val:%s, atom_loss_val:%s, bond_loss_val:%s" % (total_loss_val, mdn_loss_val,affi_loss_val,
                                                                                atom_loss_val, bond_loss_val))

total_loss_train, mdn_loss_train, affi_loss_train, atom_loss_train, bond_loss_train = run_an_eval_epoch(model, train_loader, dist_threhold=args["val_dist_threhold"], affi_weight=input_args.affi_weight , aux_weight=args["aux_weight"], device=args["device"])
logger.info("total_loss_train:%s, mdn_loss_train:%s, affi_loss_train:%s, atom_loss_train:%s, bond_loss_train:%s"%(total_loss_train, mdn_loss_train,affi_loss_train, atom_loss_train, bond_loss_train))
